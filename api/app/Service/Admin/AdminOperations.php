...
class AdminOperations
{
    ...
    public function extendTrial(Request $request)
    {
        $request->validate([
            'user_id' => 'required|integer|exists:users,id',
            'number_of_day' => 'required_without:trial_ends_at|integer|min:1|max:14',
            'trial_ends_at' => 'sometimes|date|after:now|before_or_equal:'.now()->addDays(14)->toIso8601String()
        ]);
        $user = User::findOrFail($request->get("user_id"));
        ...
        $trialEndDate = $request->filled('trial_ends_at')
    ...
    public function refundPayment(Request $request)
    {
        ...
        try {
            ...
            ], $request->attributes->get('admin_api_action_id')
