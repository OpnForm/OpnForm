...
When a form has a post-submit redirect, `redirect_url` is only returned after the URL is rendered with submitted values and passes safety checks. External redirects set `redirect_external` to `true`; the official client shows a submitted state and requires an explicit user action instead of redirecting automatically.
