...
Cloud instances expose the OpnForm MCP endpoint automatically. Self-hosted instances must opt in. See the [MCP integration guide](../integrations/mcp) for the guest and OAuth workflows.

| Variable Name | Default | Description |
| --- | --- | --- |
| `MCP_ENABLED` | `false` | Enables `/mcp` and MCP OAuth routes on self-hosted instances. |
...
| `MCP_RATE_LIMIT_PER_MINUTE` | `120` | MCP requests allowed per minute per authenticated user or guest IP. |
| `MCP_RATE_LIMIT_PER_HOUR` | `3000` | MCP requests allowed per hour per authenticated user or guest IP. |
| `MCP_DRAFT_CREATES_PER_MINUTE` | `20` | Guest drafts allowed per minute per IP. |
| `MCP_DRAFT_CREATES_PER_HOUR` | `200` | Guest drafts allowed per hour per IP. |
