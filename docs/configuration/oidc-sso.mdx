...
OpnForm always generates and validates an OIDC `state` value for login redirects. Callback requests without a valid, unexpired state are rejected to prevent login CSRF.
